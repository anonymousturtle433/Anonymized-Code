...
"""
python script for running a hyperparameter search over rnn configurations. Detailed notes on each part of the
script have been included below
"""
...
parser.add_argument("--pretrained_path", default='pretrained_models', type=str, help="Directory in which saved pretrained models are")
...
#   dataset_search_space = ['h', 's', 'sa', 'ha']
  ...
  #     'num_hidden_layers': [None],
  #     'pretrained_names':[None,
  #                         'd-sa_a-bert_lr-0.0001_e-2_g-True',
  #                         'd-s_a-bert_lr-0.0001_e-2_g-True',
  #                         'd-sa_a-bert_lr-5e-05_e-2_g-True',
  #                         'd-s_a-bert_lr-5e-05_e-2_g-True'], #name of pretrained models to use; if not None, will checked specified folder
  ...
  bert_search_space = {
      ...
      "hidden_size": [None],
      "attention": [None],
      "criterion": [combined_loss,MSE_criterion,cross_entropy_criterion,ordinal_loss],#use the keys from above
      'num_attention_heads': [None],
      'num_hidden_layers': [None],
      # name of pretrained models to use; if not None, will checked specified folder
      # directory can be defined using "pretrained_path" parameter
      'pretrained_names':[None],
                          #'d-sa_a-bert_lr-0.0001_e-2_g-True_c-False',
                          #'d-s_a-bert_lr-0.0001_e-2_g-True_c-False',
                          #'d-sa_a-bert_lr-1e-05_e-2_g-True_c-False',
                          #'d-s_a-bert_lr-1e-05_e-2_g-True_c-False'],
      # type of selection to be encoded.
      # All of them can be found in the collate function
      'selection_type': ['text_selections_simple_map','full_one_hot_with_map','partial_one_hot_with_troops_with_map'],
      'selection_output_size': [32],
      'use_text_encoder': [True],
      'goal_tokens': [True], # whether or not to add per goal classification tokens to the bert model
      # We should keep this as true which keeps the range of each bucket the same
      'uniform_buckets':[True],
      'mse_weight':[(0,1), (0.2,0.8)], #(initial weight, final weight) for the MSE metric when using combined loss
      'coral':[False],
      'mse_annealing': ['linear','logistic']
  ...
  for dataset_name in dataset_search_space:
    ...
    for arch_name in architecture_search_space:
      ...
      if arch_name == 'rnn':
        ...
        initialize_variables(vocab)  # function from training utils, required for collate function to work properly.
...
#         src_vocab = vocab.stoi
      ...
      for num, conf in enumerate(itertools.product(search_space["criterion"],
        ...
        if (params['selection_type'] == None or 'text' in params['selection_type']) and not params['use_text_encoder']:
        ...
        if params['criterion_name'] == 'ce' and params['mse_weight'][0] != 0 and params['mse_weight'][1] != 1 :
        ...
        if params['criterion_name'] != 'ord' and params['coral']:
          print('Skipping parameter configuration since we can not use coral without using an ordinal model')
          continue
        ...
        train_dataloader = DataLoader(train_dataset, batch_size=params['batch_size'], collate_fn=partial(collate_fn, tokenizer=current_tokenizer, selection_type=params['selection_type'], goal_tokens=params['goal_tokens']), shuffle=True)
        val_dataloader = DataLoader(valid_dataset, batch_size=params['batch_size'], collate_fn=partial(collate_fn, tokenizer=current_tokenizer, selection_type=params['selection_type'], goal_tokens=params['goal_tokens']))
        test_dataloader = DataLoader(test_dataset, batch_size=params['batch_size'], collate_fn=partial(collate_fn, tokenizer=current_tokenizer, selection_type=params['selection_type'], goal_tokens=params['goal_tokens']))
        ...
        if arch_name=='rnn':
          enc = Encoder(params['input_dim'], params['enc_emb_dim'], params['enc_hidden_dim'], params['enc_dropout'])
          attns = nn.ModuleList()
          ...
          if params['selection_type'] != None and 'text' not in params['selection_type']:
            selection_encoder = BertSelectionsEncoder(SELECTION_TYPE_TO_HIDDEN_DIM[params['selection_type']],
          ...
          if isinstance(criterion, nn.CrossEntropyLoss) \
            ...
            classifier = BertClassifier(HIDDEN_STATE_SIZE, NUM_GOALS, NUM_GOAL_BUCKETS, DROPOUT, is_ordinal=is_ordinal, is_coral=params['coral'])
            # initialize model weights just for untrained head
            classifier.apply(init_weights)
            model = BertToGoalClass(bert_model, classifier, selection_encoder, goal_tokens=params['goal_tokens']).to(device)
...
#                 bert_model.resize_token_embeddings(len(current_tokenizer)) #make sure that bert embedding is expanded before loading
              ...
              if params['selection_type'] != None and 'text' not in params['selection_type']: #if using selection encoder
                #load all pretrained weights from encoder (bert), but not from head
                pretrained_model_dict = {k: v for k, v in pretrained_model_dict.items() if k in model_dict and 'classifier' not in k}
          ...
          custom_config = RobertaConfig(num_attention_heads=NUM_ATTENTION_HEADS, num_hidden_layers=NUM_HIDDEN_LAYERS,
          ...
          if isinstance(criterion, nn.CrossEntropyLoss) or \
            ...
            classifier = BertClassifier(HIDDEN_STATE_SIZE, NUM_GOALS, NUM_GOAL_BUCKETS, DROPOUT, is_ordinal=is_ordinal)
        ...
        record_metrics(writer, metrics, evaluation_bucket_nums, training_metric_results, validation_metric_results, step=0)
        ...
        for epoch in range(params['num_epochs']):
          current_mse_weight = calculate_mse_weight(epoch, params['num_epochs'], initial_mse_weight, final_mse_weight, params['mse_annealing'])
          ...
          print_metrics(epoch + 1, training_metric_results, validation_metric_results, epoch_mins=epoch_mins, epoch_secs=epoch_secs)
          ...
          if valid_acc > best_acc_and_model[0]:
            ...
            best_acc_and_model = [valid_acc, log_file, epoch + 1]
          

          #write all training information to tensorboard
          record_metrics(writer, metrics, evaluation_bucket_nums, training_metric_results, validation_metric_results, step=epoch + 1)
        ...
        if args.save_pretrained_models:
          ...
          coral = params['coral']
          torch.save(model.state_dict(), f'{args.pretrained_path}/d-{dataset_name}_a-{arch_name}_lr-{lr}_e-{epochs}_g-{g_toks}_c-{coral}.pt')
  ...
  best_acc_model_name = best_acc_and_model[1]
  best_acc_model_epoch = best_acc_and_model[2]
  ...
  os.rename(f'{args.save_path}/best_acc_model.pt', f'{args.save_path}/best_acc_model_{best_acc_model_name},saved_epoch-{best_acc_model_epoch}.pt')
