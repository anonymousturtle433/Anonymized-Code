...
def call_trainer(args, num, conf, kfold=False, fold=-1, best_valid_acc=None, train_accs=None, valid_accs=None):

    if not kfold:
        assert fold == -1, "fold should not be set when kfold is false"
        assert best_valid_acc is None, "Best valid acc should not be set if kfold is false"
        assert train_accs is None, "train_accs list should be none when kfold is false"
        assert valid_accs is None, "valid_accs list should be none when kfold is false"
    else:
        assert fold > -1, "fold number must be above -1"
        assert best_valid_acc is not None, "Best valid acc should be set if kfold is true"
        assert train_accs is not None, "train_accs list should not be none when kfold is true"
        assert valid_accs is not None, "valid_accs list should not be none when kfold is true"
        best_model = None
    assert conf[11] in ['roberta-base', 'roberta-con-tokens',
                        'simple'], "The model type needs to be one of these three model configurations, ['roberta-base', roberta-con-tokens', 'simple']"
    if 'roberta' in conf[11]:
        ...
        train_dataset, valid_dataset, test_dataset = setup_datasets(args.datasets, include_unk=True,
                                                                    tokenizer=bert_tokenizer, kfold=kfold)
        if 'con-tokens' in conf[11]:
            bert_tokenizer.add_tokens(['C1', 'C2', 'C3', 'C4', 'C5', 'C6', 'C7', 'C8'])
            bert_model.resize_token_embeddings(len(bert_tokenizer))
    else:
        train_dataset, valid_dataset, test_dataset = setup_datasets(args.datasets, include_unk=True, kfold=kfold)
        bert_model = None
        bert_tokenizer = None
        bert_config = None
    ...
    if 'v1' in args.datasets or 'v2' in args.datasets or 'v3' in args.datasets:
        ...
        if 'aug' in args.datasets:
            ...
            dataset = 'synthetic'
    elif 'human_augmented' in args.datasets:
    ...
    if args.use_saved_model:
        assert params[
                   'pretrained_model'] is not None, "pretrained model cannot be none when the use saved model " \
                                                    "parameter is set to true "
    if params['pretrained_model'] is None:
        assert not args.use_saved_model, "You should not have use_saved_model set to true if pretrained_model is None"
    else:
        assert args.use_saved_model, "You should have use_saved_model set to true if pretrained_model is not None"
    ...
    if kfold:
        ...
        return best_valid_acc, deepcopy(train_accs), deepcopy(valid_accs)
    else:
        _, _ = trainer.train(num)
        del trainer.model
        torch.cuda.empty_cache()
...
if __name__ == "__main__":
    ...
    search_space = {
        "lr": [5e-4, 1e-3, 1e-4, 3.5e-5],
        "batch_size": [8, 16, 32],
        "weight_decay": [0.001],
        "enc_embedding": [40],
        "num_train_epochs": [25, 20, 15, 10],
        "optimizer": ['Adam'],
        "scheduler": ['Cosine'],
        "dropout": [0.5],
        "hidden_size": [64],
        "Glove": [False],
        "num_heads": [2],
        "model_type": ['roberta-base', 'roberta-con-tokens'],
        "pretrained_model": ['synthetic', 'synthetic_augmented'],
        "selections": [None, 'bert_no_map'],
        # "axe_delta": [0.001],
        "oaxe_c": [1.1],
        "lambda": [0.1]
    }
